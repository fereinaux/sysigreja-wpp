  ...
  private setupRoutes() {
    ...
    this.router.post("/send-text", async (req: Request, res: Response) => {
      try {
        ...
        console.log(`[Gateway] 📨 Recebida requisição para enviar mensagem de texto`);
        ...
        const status = await this.sessionManager.getSessionStatus(sessionUserId);
        console.log(`[Gateway] 📊 Status da sessão: ${status}`);

        if (status !== "connected") {
          console.error(`[Gateway] ❌ Sessão não está conectada. Status: ${status}`);
        ...
        if (!socket) {
          console.error(`[Gateway] ❌ Socket não encontrado para userId: ${sessionUserId}`);
          return res.status(404).json({
        ...
        console.log(`[Gateway] ✅ Mensagem enviada com sucesso. Message ID: ${response.messageId}`);
        res.json(response);
      } catch (error: any) {
    ...
    this.router.post("/send-image", async (req: Request, res: Response) => {
      try {
        ...
        console.log(`[Gateway] ✅ Imagem enviada com sucesso. Message ID: ${response.messageId}`);
        res.json(response);
      } catch (error: any) {
    ...
    this.router.post("/send-audio", async (req: Request, res: Response) => {
      try {
        ...
        console.log(`[Gateway] ✅ Áudio enviado com sucesso. Message ID: ${response.messageId}`);
        res.json(response);
      } catch (error: any) {
