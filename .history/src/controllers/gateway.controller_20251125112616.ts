...
export class GatewayController {
  ...
  private setupRoutes() {
    ...
    this.router.post(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          console.log(`[Gateway] ✅ Sessão criada - Status: ${result.status}, QR gerado: ${result.qr ? 'Sim' : 'Não'}`);
          res.json(result);
        } catch (error: any) {
    ...
    this.router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          console.log(`[Gateway] 📊 Status da sessão ${userId}: ${status}, QR disponível: ${qr ? 'Sim' : 'Não'}`);
          res.json(response);
        } catch (error: any) {
          console.error(`[Gateway] ❌ Erro ao verificar status:`, error.message);
          res.status(500).json({ error: error.message });
        }
      }
    );
    ...
    // Obter QR Code
    this.router.get('/sessions/:userId/qr', async (req: Request, res: Response) => {
      try {
        const { userId } = req.params;
        const qr = await this.sessionManager.getQRCode(userId);
    ...
    this.router.post('/send-image', async (req: Request, res: Response) => {
      try {
        const { sessionUserId, to, imageKey, caption }: SendImageRequest = req.body;
    ...
    // Remover sessão
    this.router.delete('/sessions/:userId', async (req: Request, res: Response) => {
      try {
        const { userId } = req.params;
