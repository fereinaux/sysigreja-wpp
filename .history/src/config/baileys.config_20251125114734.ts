...
export async function createBaileysSocket(
  ...
  // O erro 405 geralmente indica que o WhatsApp está bloqueando a conexão
  // Possíveis causas: IP de data center, firewall, ou restrições de rede
  const socket = makeWASocket({
    auth: state,
    printQRInTerminal: false,
    logger: pino({ level: "warn" }), // Logs de warning para debug
    // Configuração de browser - tentando diferentes formatos para evitar erro 405
    // Alguns usuários relataram sucesso com configurações específicas
    // Opção 1: Formato customizado (atual)
    browser: ["SysIgreja", "Chrome", "1.0.0"],
    // Opção 2: Ubuntu Chrome (mencionado em algumas issues)
    // browser: ["Ubuntu", "Chrome", "22.04.4"],
    // Opção 3: macOS Desktop (mencionado em algumas issues)
    ...
    // Configurações adicionais para tentar evitar erro 405
    // Se você tiver um proxy, pode configurar aqui usando fetchAgent
    // fetchAgent: new HttpsProxyAgent('http://proxy-url:port'),
  });
