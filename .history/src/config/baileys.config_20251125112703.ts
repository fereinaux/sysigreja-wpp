import { makeWASocket, DisconnectReason, ConnectionState, WASocket } from '@whiskeysockets/baileys';
...
export async function createBaileysSocket(
  ...
  console.log(`[Baileys] 🔧 Inicializando socket para userId: ${config.userId}`);
  const storage = new RedisStorageService(config.userId);
  const { state, saveCreds } = await storage.getState();
  console.log(`[Baileys] 📦 Estado de autenticação carregado para userId: ${config.userId}`);
  ...
  socket.ev.on('creds.update', async () => {
    console.log(`[Baileys] 💾 Salvando credenciais atualizadas para userId: ${config.userId}`);
    await saveCreds();
  });
  ...
  socket.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      console.log(`[Baileys] 📱 QR Code gerado para userId: ${config.userId} (tamanho: ${qr.length} chars)`);
      // QR Code gerado - salvar no Redis e notificar callback
      await storage.setQRCode(qr);
      console.log(`[Baileys] 💾 QR Code salvo no Redis para userId: ${config.userId}`);
      if (config.onQRCode) {
        console.log(`[Baileys] 📞 Chamando callback onQRCode para userId: ${config.userId}`);
        config.onQRCode(qr);
      }
    }
    ...
    if (connection === 'open') {
      ...
      console.log(`[Baileys] ✅ Status atualizado para 'connected' para userId: ${config.userId}`);
    ...
    if (connection === 'close') {
      ...
      console.log(`[Baileys] 🔴 Conexão fechada para userId: ${config.userId}, reconectar: ${shouldReconnect}`);
      await storage.setStatus('disconnected');

      if (shouldReconnect) {
        // Reconectar automaticamente após 3 segundos
        console.log(`[Baileys] 🔄 Agendando reconexão em 3s para userId: ${config.userId}`);
        setTimeout(() => {
          createBaileysSocket(config).catch((err) => {
            console.error(`[Baileys] ❌ Erro ao reconectar sessão ${config.userId}:`, err);
          });
        }, 3000);
      } else {
        // Logout - limpar estado
        console.log(`[Baileys] 🗑️  Limpando estado (logout) para userId: ${config.userId}`);
