...
export class SessionManager {
  ...
  async createSession(userId: string): Promise<CreateSessionResponse> {
    console.log(`[SessionManager] 🔍 Verificando sessão existente para userId: ${userId}`);
    
    // Verificar se já existe uma sessão conectada
    if (this.sessions.has(userId)) {
      console.log(`[SessionManager] ⚠️  Sessão já existe para userId: ${userId}`);
      const socket = this.sessions.get(userId)!;
      const status = await this.getSessionStatus(userId);

      if (status === 'connected') {
        console.log(`[SessionManager] ✅ Sessão já conectada para userId: ${userId}`);
      ...
      if (existingQr) {
        console.log(`[SessionManager] 📱 QR Code existente encontrado para userId: ${userId}`);
        return { qr: existingQr, status: 'qr_pending' };
      }
    }

    console.log(`[SessionManager] 🚀 Iniciando criação de nova sessão para userId: ${userId}`);
    const storage = this.getStorage(userId);
    await storage.setStatus('connecting');
    console.log(`[SessionManager] 📝 Status definido como 'connecting' para userId: ${userId}`);

    try {
      let qrCode: string | null = null;
      let qrResolved = false;

      console.log(`[SessionManager] 🔌 Criando socket Baileys para userId: ${userId}`);
      const { socket } = await createBaileysSocket({
        userId,
        onQRCode: (qr) => {
          console.log(`[SessionManager] 🎯 QR Code recebido via callback para userId: ${userId}`);
          qrCode = qr;
          qrResolved = true;
        },
        onConnectionUpdate: async (update) => {
          if (update.connection === 'open') {
            console.log(`[SessionManager] ✅ Conexão estabelecida para userId: ${userId}`);
            this.sessions.set(userId, socket);
            await storage.setStatus('connected');
          } else if (update.connection === 'close') {
            console.log(`[SessionManager] 🔴 Conexão fechada para userId: ${userId}`);
      ...
      console.log(`[SessionManager] ⏳ Aguardando QR Code ser gerado (timeout: 10s) para userId: ${userId}`);
      // Aguardar QR Code ser gerado (timeout de 10s)
      const qrPromise = new Promise<string | null>((resolve) => {
        const checkInterval = setInterval(() => {
          if (qrResolved) {
            clearInterval(checkInterval);
            console.log(`[SessionManager] ✅ QR Code resolvido para userId: ${userId}`);
            resolve(qrCode);
          }
        }, 100);
        ...
        setTimeout(() => {
          clearInterval(checkInterval);
          if (!qrResolved) {
            console.log(`[SessionManager] ⏰ Timeout aguardando QR Code para userId: ${userId}`);
            resolve(null);
          }
        }, 10000);
      });
      ...
      if (qr) {
        console.log(`[SessionManager] ✅ QR Code gerado com sucesso para userId: ${userId} (tamanho: ${qr.length} chars)`);
      ...
      if (currentStatus === 'connected') {
        console.log(`[SessionManager] ✅ Sessão já conectada (verificação pós-timeout) para userId: ${userId}`);
        this.sessions.set(userId, socket);
        return { qr: null, status: 'connected' };
      }

      console.log(`[SessionManager] ⚠️  Timeout - QR Code não foi gerado para userId: ${userId}`);
      return { qr: null, status: 'timeout' };
    } catch (error: any) {
      console.error(`[SessionManager] ❌ Erro ao criar sessão para userId: ${userId}:`, error);
      await storage.setStatus('disconnected');
