...
export class MessageSender {
  ...
  async sendText(
    ...
    console.log(
      `[MessageSender] 💬 Mensagem: ${message.substring(0, 50)}${
        message.length > 50 ? "..." : ""
      }`
    );
  ...
  async sendImage(
    ...
    try {
      ...
      console.log(
        `[MessageSender] 🔍 Verificando se imagem existe no MinIO...`
      );
      const exists = await this.minioClient.objectExists(imageKey);
      if (!exists) {
        console.error(
          `[MessageSender] ❌ Imagem não encontrada no MinIO: ${imageKey}`
        );
      ...
      console.log(
        `[MessageSender] ✅ Imagem baixada (tamanho: ${imageBuffer.length} bytes)`
      );
  ...
  async sendAudio(
    ...
    try {
      ...
      if (!exists) {
        console.error(
          `[MessageSender] ❌ Áudio não encontrado no MinIO: ${audioKey}`
        );
      ...
      console.log(
        `[MessageSender] ✅ Áudio baixado (tamanho: ${audioBuffer.length} bytes)`
      );
